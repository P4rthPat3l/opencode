...
describe("prompt submit worktree selection", () => {
  ...
  test("requests fresh IDE editor context for normal prompts", async () => {
    ...
    const legacyParts = (
      promptInputs[0] as { legacyParts: Array<{ type: string; text?: string; synthetic?: boolean }> }
    ).legacyParts

    expect(legacyParts).toContainEqual(
    ...
    expect(legacyParts.find((part) => part.synthetic)?.text).toContain("- src/b.ts")
  })
